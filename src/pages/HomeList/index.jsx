...
const HomeList = () => {
    ...
    const saveProduct = async () => {
        ...
        if (product.name.trim()) {
            console.log(product);
            const submitData = await request('/api/home/add', 'POST', JSON.stringify(product))
    ...
    const confirmEditProduct = async () => {
        console.log(product);
        const submitData = await request(`/api/home/update`, 'POST', JSON.stringify({
            id: product.id,
            name: product.name,
    ...
    const ownerName = (rowData) => {
        return (
            <div>
                {rowData.owner && rowData.owner.fullName}
            </div>
        )
    }
        ...
        <div>
            ...
            <div className="card">
                ...
                <DataTable ref={dt} value={products} selection={selectedProducts} onSelectionChange={(e) => setSelectedProducts(e.value)}
                    ...
                    <Column field="address" body={homeName} header="địa chỉ nhà" style={{ minWidth: '12rem' }}></Column>
                    <Column field="" body={ownerName} header="Chủ nhà" style={{ minWidth: '16rem' }}></Column>
                    {/* <Column field="member" body={memberCount} header="Thành viên" style={{ minWidth: '12rem' }}></Column> */}
                    <Column field="electricPrice" header="Giá điện" style={{ minWidth: '12rem' }}></Column>
                    <Column header="Thao tác" body={actionBodyTemplate} exportable={false} style={{ minWidth: '12rem' }}></Column>

                </DataTable>
            </div>

            <Dialog visible={productDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Add home" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>
                <div className="field">
            ...
            <Dialog visible={editProductDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Add home" modal className="p-fluid" footer={editDialogFooter} onHide={hideEditDialog}>
                <div className="field">
                    <label htmlFor="name" className="font-bold">
                        CCCD chủ nhà
                    </label>
